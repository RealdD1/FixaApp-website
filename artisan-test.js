...
(function () {
  ...
  function initSocket() {
    try {
      ...
      // In initSocket(), add these with the other socket.on() listeners:
socket.on('topUpPaid', (data) => {
  showStatusMessage(`✅ Customer paid the top-up of ₦${(data.amount||0).toLocaleString()}`, 'success');
  // Refresh phase in case amount changed
  if (activeChatId) loadArtisanChatPhase(activeChatId);
});

socket.on('topUpRejected', () => {
  showStatusMessage('Customer rejected the top-up request', 'warning');
  // Re-enable the top-up button
  const btn = document.getElementById('topUpRequestBtn');
  ...
  function applyArtisanChatStateToUI() {
   ...
   if (disputeBtn) {
    // Only show "Report a problem" if artisan wants to escalate something
    // If there's already an open dispute filed by customer, show different state
    const canDispute = activeBookingId && (phase === 'secured' || activeChatStatus === 'completed');

    if (activeBooking?.disputeStatus === 'open') {
      // Customer filed a dispute — show passive state
      disputeBtn.style.display = 'inline-block';
      disputeBtn.textContent = '🚨 Under Review';
      disputeBtn.style.background = 'rgba(239,68,68,0.25)';
      disputeBtn.style.cursor = 'default';
      disputeBtn.title = 'Customer filed a dispute. Fixa is reviewing.';
      disputeBtn.onclick = () => {
        showStatusMessage('Customer filed a dispute. Fixa is reviewing — please be patient.', 'warning');
      };
    } else {
      disputeBtn.style.display = canDispute ? 'inline-block' : 'none';
      disputeBtn.textContent = '🚨';
      disputeBtn.title = 'Report a problem';
      disputeBtn.onclick = () => {
        if (!activeBookingId) {
  ...
  if (topUpBtn) {
    const canTopUp = ['started', 'on_the_way'].includes(activeExecutionStatus);
    topUpBtn.style.display = canTopUp ? 'block' : 'none';
  }
  }
...
async function submitTopUpRequest() {
  ...
  if (!amount || amount < 100) {
    showStatusMessage('Enter a valid amount (min ₦100)', 'warning');
    return;
  }
  if (reason.length < 10) {
  ...
  try {
    ...
    showStatusMessage('Top-up request sent. Waiting for customer approval.', 'success');
  } catch (err) {
...
fetch(`${MESSAGE_API_URL}/${encodeURIComponent(chatId)}/read`, {
  method: 'POST',
  headers: authHeaders()
}).catch(() => {});
  function closeChat() {
...
document.getElementById('topUpCloseBtn')?.addEventListener('click', () => {
  document.getElementById('topUpModal').style.display = 'none';
});
document.getElementById('topUpCancelBtn')?.addEventListener('click', () => {
  document.getElementById('topUpModal').style.display = 'none';
});
document.getElementById('topUpSubmitBtn')?.addEventListener('click', submitTopUpRequest);
